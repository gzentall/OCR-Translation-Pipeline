...
# Read the Vision API key from environment variable or .gcp_api_key file
if [[ -n "${GCP_VISION_API_KEY:-}" ]]; then
    echo "[run_vision_ocr] Using API key from environment variable"
else
    API_KEY_FILE="$PROJECT_ROOT/.gcp_api_key"
    if [[ ! -f "$API_KEY_FILE" ]]; then
        echo "Missing GCP_VISION_API_KEY environment variable and $API_KEY_FILE file." >&2
        echo "Please set GCP_VISION_API_KEY or place your Google Vision API key in .gcp_api_key" >&2
        exit 1
    fi
    ...
    echo "[run_vision_ocr] Using API key from file"
fi
